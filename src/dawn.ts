...
export function parseAndRenderDawnfile(fileContent: string) {
  ...
  let forceWireframe = true; // All 3D things transparent
  ...
  lines.forEach((line) => {
    ...
    switch (command) {
      ...
      case '/Cons': {
        let [rmin1, rmax1, rmin2, rmax2, dz, sphi, dphi] = parts
          .slice(1)
          .map((p) => parseFloat(p));

        if (
          [rmin1, rmax1, rmin2, rmax2, dz, sphi, dphi].every(
            (v) => !isNaN(v)
          ) &&
          dz >= 0
        ) {
      ...
      case '/Torus': {
        let [rmin, rmax, rtor, sphi, dphi] = parts
          .slice(1)
          .map((p) => parseFloat(p));
      ...
      case '/Tubs': {
        let [rmin, rmax, dz, sphi, dphi] = parts
          .slice(1)
          .map((p) => parseFloat(p));

        if (
          [rmin, rmax, dz, sphi, dphi].every((v) => !isNaN(v)) &&
          rmax >= 0 &&
          dz >= 0
        ) {
          rmin = Math.max(0, rmin);
